...
const mainLoadFn = async () => {
  ...
  try {
    ...
    const totalChunks = Math.ceil(
      newCourseLessonResourcesForLMS.length / MAX_RECORDS_TO_INSERT,
    );
    for (
      let i = 0, cont = 1;
      i < newCourseLessonResourcesForLMS.length;
      i += MAX_RECORDS_TO_INSERT, cont++
    ) {
      chunk = newCourseLessonResourcesForLMS.slice(
        i,
        i + MAX_RECORDS_TO_INSERT,
      );
 ...
 */
export const startCatalogCourseLessonsResourcesPipeline = async () => {
  console.log('\n**************************************');
